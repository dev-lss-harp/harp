...
(function ($) {
    $.fn.HarpFilter = function (options) {
        var defaults = {
            ...
            'logicalOperatorWidth': '100%',
            'relationalOperatorWidth': '100%',
            'logicalOperatorMinWidth': '100%',
            'logicalOperatorMaxWidth': '100%',
            'filterFieldMinWidth': '100%',
            'filterFieldMaxWidth': '100%',
            'btnAddFilter': true,
        ...
        jQuery('head').append('<script type="text/javascript" src="' + settings.baseUrl + '/HarpFilter/js/md5-js/md5.js"></script>');
                jQuery('head').append('<script type="text/javascript" src="' + settings.baseUrl + '/HarpFilter/js/jquery-browser-fingerprint/src/jquery.browser-fingerprint-1.1.min.js"></script>');
        ...
        function loaderGif(elementBindClick) 
        {
            if (jQuery(settings.loadWhileSubmit.elementImage).length > 0) 
            {
                ...
                jQuery(settings.loadWhileSubmit.elementImage).addClass('btn');
        ...
        var fingerPrintHash = jQuery.fingerprint();
       
        jQuery('input[name="' + idElement + '-fingerPrint"]').val(fingerPrintHash);
  
        var currentFilter = md5(idElement + fingerPrintHash);
        ...
        jQuery('#' + idElement).append('<div class=""><div class="ContainerElement"></div></div>');
        jQuery('#' + idElement).append('<button class="buttonAddFilter" style="margin-left:0.2%;"><i class="material-icons" style="color:#FFFFFF;">&#xE145;</i></button>');
        if (settings.btnCleanAllFilters) 
        {
            jQuery('#' + idElement).append('<button class="cleanAllFilter" style="margin-bottom:0.2%;"><i class="material-icons" style="color:#FFFFFF;">&#xE92B;</i></button>');
        }
        jQuery('#' + idElement).append('<button class="buttonSubmitFilter" style="margin-bottom:0.2%;"><i class="material-icons" style="color:#FFFFFF;">&#xE8B6;</i></button>');
        jQuery('#' + idElement).append('<span class="showLoaderImage" style="float:left;display:inline-block;border:none;background:none;outline:none;transition:none;z-index:1;border-radius:0;cursor: not-allowed;box-shadow:none;"></span>');


        jQuery('#' + idElement).append('<div class="" style="margin:0.1%;"><div class="ActiveFilters"></div></div>');
        ...
        jQuery('body').on('click', 'button,a,input[type="submit"]', function (e) {
            if (jQuery(this).attr('class') != 'buttonSubmitFilter') {
                ...
                else 
                {
                    ...
                    //console.log(elementAdrress);
                    testUri = (typeof elementAddress != 'undefined') ? ((elementAdrress == '#' || elementAdrress.trim() == '') ? false : true) : false;
                }
        ...
        function showOrHideButtons(s) {
            if (settings.btnAddFilter) {
                (s === true) ? jQuery('#' + idElement + ' .buttonAddFilter').show() : jQuery('#' + idElement + ' .buttonAddFilter').hide();
            }

            (s === true) ? jQuery('#' + idElement + ' .buttonSubmitFilter').show() : jQuery('#' + idElement + ' .buttonSubmitFilter').hide();
        }

        function changeRelationalOperator(_this)
        {
        ...
        function changeFilterField(_this) {
        ...
        function cloneElements(s) {
            if (jQuery("#" + filterFieldClass).length > 0) {
                filterElementSelected = jQuery("#" + filterFieldClass).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + filterFieldClass).hide();
                ...
                filterElementSelected = jQuery('input[id^="' + filterFieldClass + '-"]').eq(0).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + filterFieldClass + '-1').hide();
                filterElementSelected2 = jQuery('input[id^="' + filterFieldClass + '-"]').eq(1).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + filterFieldClass + '-2').hide();
            ...
            if (s) {
                ...
                if (filterElementSelected2 != null && filterElementSelected2 != '' && typeof filterElementSelected2 != 'undefined') {
                    jQuery(filterElementSelected2).show();
                }
        ...
        function enabledFirstRelationalOperator()
        {
            jQuery('#' + idElement + ' .relationalOperator select option').each(function (a, b) {
                ...
                if (!d)
                {
                    jQuery('#' + idElement + ' .relationalOperator > select').val(jQuery(b).val()).trigger('change');
                   // jQuery('#' + idElement + ' > .relationalOperator > select')
        ...
        function validateFilterTypeValue() {
            ...
            if (type === 'int' || type === 'bit') {
                strCommand = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + formatedElement + ' ';
                ...
                keyParam = '@p' + md5(fullStrDisplay);

                param = keyParam;

                if (relationalOperatorValue == 'BETWEEN' || relationalOperatorValue == 'between') {
                    strCommand = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + formatedElement + (formatedElement2 == null ? ' ' : ' AND ' + formatedElement2);
                ...
                if (relationalOperatorValue == 'BETWEEN' || relationalOperatorValue == 'between') {
                    strCommand = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + pCommand + ' ' + (pCommand2 != null ? 'AND ' + pCommand2 : '');
                }
                else {
                    strCommand = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + pCommand;
                }
            }

            if (relationalOperatorValue == 'BETWEEN' || relationalOperatorValue == 'between') {
                strCommandParameter = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + param + ' ' + (pCommand2 != null ? 'AND ' + param2 : '');
            }
            else {
                strCommandParameter = (' ' + logicalOperatorValue + ' ' + filterFieldValue + ' ' + relationalOperatorValue) + ' ' + param;
            }
        ...
        return this.each(function () {
                ...
                if (lo && ro && ff && tickets.logicalOperator != '' && tickets.relationalOperator != '' && tickets.filterField != '') {
                    ...
                    if (typeof idElementCurrent != 'undefined') {
                        ...
                        if (jQuery('input[id^="' + idElementCurrent + '-"]').length > 0) {
                            cln = jQuery('input[id^="' + idElementCurrent + '-"]').eq(0).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + idElementCurrent + '-1');
                            cln1 = jQuery('input[id^="' + idElementCurrent + '-"]').eq(1).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + idElementCurrent + '-2');
                            jQuery('#' + idElement + ' .ContainerElement').empty().append(cln).append(cln1).show();
                        }
                        else {
                            cln = jQuery("#" + idElementCurrent).clone(true,true).prop('disabled', false).removeAttr('id').addClass('cloned-element').attr('id', 'cloned-element-' + idElementCurrent);
            ...
            function addFilter() {
                ...
                if (typeof jsonObject.filter[keyParam] === 'undefined') {
                    ...
                    jQuery('#' + idElement + ' .ActiveFilters').append('<span class="label-ticket" style="padding:0.1%;"><div>' + strDisplay + '</div><button value="' + keyParam + '"><img src="' + settings.baseUrl + '/HarpFilter/img/x.png" width="16px"></button></span>');
            ...
            jQuery('body').on('click', '#' + idElement + ' .ActiveFilters .label-ticket button', function (e) 
            {
                ...
                if (typeof jsonObject.filter[jQuery(this).val()] != 'undefined') 
                {
                    ...
                    if (typeof (Storage) != "undefined") 
                    {
                          //console.log(currentFilter,jQuery('#' + idElement + ' .filterData').val());return false;
                        localStorage.setItem(currentFilter, jQuery('#' + idElement + ' .filterData').val());
                    }
            ...
            jQuery('#' + idElement + ' .buttonSubmitFilter').click(function (e) {
                ...
                else if (jQuery(filterElementSelected).length > 0) {//console.log(tickets.length);return false;
            ...
            jQuery('#' + idElement + ' .relationalOperator  select').change(function () {
                changeRelationalOperator(jQuery(this));
                showOrHideButtons(true);
            });

            jQuery('#' + idElement + ' .filterField select').change(function () 
            {
                changeFilterField(jQuery(this));

                enabledRelationalOperators(filterFieldValue);

                var sf = (filterFieldValue.trim() != null && filterFieldValue.trim() != '' && typeof filterFieldValue != 'undefined');

                if (settings.enableDefaultSearchOnChangeFilterField && sf) 
                {
                    cloneElements(true);
