...
namespace etc\HarpDesignTemplate\components\HandleHtml\HandleI;
...
class IElement
{
    ...
    public function __construct(HandleI $HandleI) 
    {                  
    ...
    { 
        ...
        preg_match_all($regex,$this->HandleI->getFileDocument()->getFile()->file,$result);
    ...
    public function replaceContentById($id,$content)
    {
        ...
        if(!empty($element[0]) && !empty($element[1]))
        {
            foreach($element[0] as $i => $v)
            {
                ...
                $this->HandleI->getFileDocument()->getFile()->file = str_ireplace($v, $el,$this->HandleI->getFileDocument()->getFile()->file);
            }
